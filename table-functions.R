...
hr_tbl <- function(name, expo_var, out_var, exposure, outcome, results, results_adj, adj_only=F){
  ...
  if(adj_only){
    tbl <- data.table(name = character(), "Outcome" = character(), "N" = character(), "25th Percentile" = character(), "75th Percentile" = character(),
                      ...
                      " " = character(), " " = character())
    tbl <- rbind(tbl, list(" ", " ", " ", " ", " ", "Adjusted", " ", " "))
    tbl <- rbind(tbl, list(" ", " ", " ", " ", " ", "Hazard Ratio (95% CI)", "P-value", "FDR adjusted P-value"))
    skipped<-F
    for (i in 1:length(exposure)) {
      for (j in 1:length(outcome)) {
        exp <- exposure[i]
...
hr_tbl_flex <- function(name, expo_var, out_var, exposure, outcome, results, results_adj, adj_only=F){
  ### name: string name of group of exposures
  ### expo_var: vector of string exposures to include in table
  ### out_var: vector of string outcomes to include in table
  ### exposure: vector of string exposure variable names
  ...
  if(adj_only){
    ...
    flextbl <- add_header_row(flextbl, values = c("","","","","", "Adjusted"), colwidths=c(1,1,1,1,1,3))
    flextbl <- add_header_row(flextbl, values = c(name, "Outcome","N","25th Percentile","75th Percentile", "Outcome, 75th Percentile v. 25th Percentile"), colwidths=c(1,1,1,1,1,3))
  ...
  if(adj_only){
    flextbl <- add_footer_row(flextbl, top=F, 
                              values = "N, 25th Percentile, and 75th Percentile are from the adjusted analyses", colwidths = 8)
  }else{
    flextbl <- add_footer_row(flextbl, top=F, 
